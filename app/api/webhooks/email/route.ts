...
export async function POST(req: Request) {
  ...
  try {
    ...
    // If this user has a WORKING email connection (Gmail/Outlook) pulling their
    // school mail directly, skip the forwarded copy to avoid double-processing
    // the same email. A connection only counts if it is configured (school
    // senders set) AND not currently failing: a dead refresh token used to
    // suppress the forwarded copy too, so a broken connector took the fallback
    // down with it and the parent got nothing from either route.
    // select('*') because last_error may not exist yet on older databases;
    ...
    const [{ data: gmailConn }, { data: outlookConn }] = await Promise.all([
      ...
      supabase.from('outlook_connections').select('*').eq('user_id', user.id).maybeSingle()
    ])
    const isUsable = (conn: any) => (conn?.school_domains?.length ?? 0) > 0 && !conn?.last_error
    const hasActiveConnection = isUsable(gmailConn) || isUsable(outlookConn)
    if (hasActiveConnection) {
      console.log('User has a working email connection — skipping forwarded copy to avoid double-processing')
